...
public class CopyOfOozie extends Configured implements Tool {
	...
	public void run() {
		try {
			...
			p.load(CopyOfOozie.class.getResourceAsStream("co.properties"));
	...
	public static void main(String[] args) {
		...
		elist.add(e2);
		CopyOfOozie oozie = new CopyOfOozie();
		oozie.initialize(elist);
		//oozie.start();
		
		ToolRunner runner=new ToolRunner();
		try {
			runner.run(oozie, null);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private void initialize(List<Entity> elist) {
		...
		try {
			...
			for (Entity entity : elist) {
				String s=IOUtils.toString(CopyOfOozie.class.getResourceAsStream("ds.xml"));
				s=StringUtils.replace(s, "#1", entity.getId());
				ds.append(StringUtils.replace(s, "#2", entity.getName()));
		
				String i=IOUtils.toString(CopyOfOozie.class.getResourceAsStream("ie.xml"));
			...
			co=IOUtils.toString(CopyOfOozie.class.getResourceAsStream("co.xml"));
			co=StringUtils.replace(co, "#1", ds.toString());
			co=StringUtils.replace(co, "#2", ie.toString());
			System.out.println("co="+co); 
			fc=IOUtils.toString(CopyOfOozie.class.getResourceAsStream("fc.xml"));
